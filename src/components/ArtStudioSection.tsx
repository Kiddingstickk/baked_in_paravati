...
import artStudioImage from "../assets/paul-pastourmatzis-r0J9sGBWFOc-unsplash.jpg";       // replace with your actual image
...
export default function ArtStudioSection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(
        headingRef.current,
        { xPercent: 120, opacity: 0 }, // slide in from right
        {
          xPercent: 0,
          opacity: 1,
          ease: "power2.out",
  ...
  return (
    <>
      ...
      >
        ...
        {/* Layer 2: Heading + Content (right aligned) */}
        <div
          ref={headingRef}
          className="absolute inset-0 z-10 flex items-start justify-end px-12 pt-40 text-right"
        >
          <div className="inline-block translate-x-3" style={{ transform: "translateZ(0)" }}>
            <h1 className="font-serif text-[6.5vw] md:text-[5vw] font-bold text-white leading-none tracking-wide">
              ART STUDIO EXPERIENCE
            </h1>
            <p className="mt-6 text-gray-200 max-w-md text-lg">
              Baked in Parvati – Art Studio, Pulga.  
              An immersive creative stay where nature and artistry blend.  
              Perfect for those seeking inspiration and soulful expression.
            </p>
          </div>
        </div>
      ...
      <section className="relative min-h-[100vh] w-full overflow-hidden block md:hidden">
        ...
        <div className="absolute inset-0 z-10 flex flex-col items-start justify-center px-6 text-left">
          <h1 className="font-serif text-[10vw] font-bold text-white leading-none tracking-wide">
            ART STUDIO EXPERIENCE
          </h1>
